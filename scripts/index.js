    ...
    if (inputElement) {
        inputElement.addEventListener('input', async function() {
            ...
            try {
                ...
                var result = await stardict.query(vcb); // 确保 'word' 是一个字符串
...
function clearDefinitions() {
    document.getElementById('enDef').textContent = '';
    document.getElementById('cnDef').textContent = '';
}
...
function processGlobalQueryResult(result) {
    if (!result || result === null) {
        ...
        document.getElementById('enDef').textContent = result.definition || 'No definition found.';
        document.getElementById('cnDef').textContent = result.translation || '没有找到翻译。';
    }
}
